...
def main(input_path: Path, output_path: Path) -> None:
    ...
    # Génération automatique de la version anonymisée (sans noms) pour publication
    CHAMPS_PUBLICS = {"generation", "commune", "departement", "annee_naissance", "lat", "lon"}
    anonymises = [
        {k: v for k, v in p.items() if k in CHAMPS_PUBLICS}
        for p in enrichies
    ]
    public_path = output_path.parent.parent / "public" / "arbre_geocoded.json"
    public_path.parent.mkdir(parents=True, exist_ok=True)
    with open(public_path, "w", encoding="utf-8") as f:
        json.dump(anonymises, f, ensure_ascii=False, indent=2)
    print(f"\n✓ Version anonymisée écrite : {public_path}")
